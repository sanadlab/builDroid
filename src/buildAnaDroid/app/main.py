...
def run_interaction_loop(
    ...
    image_log = ""
    if not check_image_exists("build-anadroid:0.4.0"):
        ...
        with open("tests/Dockerfile", "w", encoding="utf-8") as f:
            f.write(dockerfile)
        image_log = build_image("tests", "build-anadroid:0.4.0")
        if image_log.startswith("An error occurred while building the Docker image"):
            print(image_log)
            sys.exit(1)
    agent.container = start_container(f"build-anadroid:0.4.0", f"{agent.project_path[:63]}")
